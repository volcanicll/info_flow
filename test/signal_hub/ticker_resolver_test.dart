...
void main() {
  ...
  Article a0(String title, {String? content}) => Article(
        id: 'x',
        feedId: 'f',
        feedName: 'n',
        title: title,
  ...
  test('标题含 ETH，正文多次出现，返回 ETH 且 inTitle=true', () {
    final a = a0('以太坊上海升级临近', content: 'ETH 将升级，以太坊社区热议。ETH ETH。');
  ...
  test('同时命中多个标的，按 inTitle 优先、mentions 次之排序', () {
    final a = a0('BTC 与 eth', content: '比特币 bitcoin 狗狗币 doge');
    final refs = resolver.resolve(a);
  ...
  test('无任何标的出现时返回空列表', () {
    final a = a0('某地天气晴朗', content: '今天适合散步');
    expect(resolver.resolve(a), isEmpty);
  });

  test('「苹果」非财经语境不应误判（P0 词典无美股，天然不命中）', () {
    final a = a0('苹果好吃', content: '今天吃了两个苹果');
    expect(resolver.resolve(a), isEmpty);
  });

  test('同 symbol 多别名命中合并 mentions', () {
    final a = a0('ETH', content: '以太坊 以太 eth');
    final refs = resolver.resolve(a);
    final eth = refs.where((r) => r.symbol == 'ETH').single;
    expect(eth.mentions, greaterThanOrEqualTo(3));
  });
